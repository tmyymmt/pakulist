...
| 時刻 | スライド | 投影する主張 | 話すこと |
| --- | --- | --- | --- |
| 0:00–0:18 | 1. 課題 | SNS投稿の重複を見つけるだけでは対応できない | 候補、根拠、最終判断を分ける必要を説明する。 |
| 0:18–0:42 | 2. サービスと成果物 | SNS投稿の候補と根拠を整理するローカルMVP | Web MVP、決定的検出、CSV/HTML出力を示す。 |
| 0:42–1:08 | 3. 無償版と有償版 | ローカルOSSと外部連携を含む有償サービスを分離 | X API、意味的類似判定、権限、利用枠の違いを示す。 |
| 1:08–1:40 | 4. なぜ完成品にしなかったか | 不確実性を計画で先に固定した | データ権限、利用者価値、AI原価、保存を先取りしない判断を伝える。 |
| 1:40–2:10 | 5. 最小プロトタイプ | 仕様を検証するためだけに実装した | 入力→検出→候補・根拠→CSV/HTMLの流れを示す。 |
| 2:10–3:25 | 6–7. 評価基準への対応 | 8基準を達成として整理し、注記で次の対応を示す | 実装・設計の証跡と後続の実測を述べる。 |
| 3:25–4:00 | 8–9. 次の一手・締め | 実装競争ではなく、承認と実測へ進む | 外部データ承認、利用者検証、料金設計、限定AI評価の順を示す。 |
